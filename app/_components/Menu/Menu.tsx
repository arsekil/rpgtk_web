...
export default function Menu() {
    ...
    return (
        <div className="ml-3 flex flex-row justify-start items-center gap-2 h-full font-bold text-sm">
            {menuItems.map((item) => (
                <div key={item.href} className={`p-2 pb-0 rounded-t-xl ${pathname === item.href ? "bg-menu-selected-bg" : ""}`}>
                    <Link href={item.href} className="text-white" title={`Go to the ${item.label} section`} onClick={() => setMenuItem(pathname || item.href)}>
                        {item.label}
                    </Link>
                </div>
            ))}
